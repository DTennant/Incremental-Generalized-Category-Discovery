...
from config import nabirds_root
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, loader=default_loader, download=False):
        ...
        if not self._check_integrity():
            raise RuntimeError('Dataset not found or corrupted.' +
                               ' You can download it from https://dl.allaboutbirds.org/nabirds, and also move ssb_processed.csv to the nabirds folder')

        self.uq_idxs = np.array(range(len(self)))

    def _load_metadata(self):

        self.data = pd.read_csv(os.path.join(self.root, 'nabirds', 'ssb_processed.csv'), names=['img_id', 'filepath', 'is_training_img', 'target', 'cls_name'])
...
def get_nabirds_datasets(train_transform, test_transform, train_classes=range(160), prop_train_labels=0.8,
    ...
    whole_training_set = NABirds(root=nabirds_root, transform=train_transform, train=True, )
    ...
    test_dataset = NABirds(root=nabirds_root, transform=test_transform, train=False)
